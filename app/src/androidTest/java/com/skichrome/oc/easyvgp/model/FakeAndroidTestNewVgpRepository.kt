...
class FakeAndroidTestNewVgpRepository(
...
) : NewVgpRepository
{
    ...
    fun insertForTestsMachineTypeAndCtrlPoints(machineTypeWithControlPoints: List<MachineTypeWithControlPoints>) =
        machineTypeWithControlPoints.forEach {
            machineTypeWithCtrlPtsDataService[it.machineType.id] = it
        }

    fun insertForTestsExtras(extras: List<MachineControlPointDataExtra>) = extras.forEach {
        extraDataService[it.id] = it
    }

    fun insertForTestsControlPointData(ctrlPtData: List<ControlPointData>) = ctrlPtData.forEach {
        ctrlPtDataService[it.id] = it
    }

    fun insertForTestsReports(reports: List<Report>) = reports.forEachIndexed { index, report ->
        reportsDataService[index.toLong()] = report
    }
}